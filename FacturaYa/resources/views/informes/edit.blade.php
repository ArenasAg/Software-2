...
<div class="container py-4">
    ...
    <h2 class="page-title">Editar Informe</h2>
        ...
        <div class="card-body">
            <form action="{{ route('informes.update', $informe->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="date" class="form-control @error('fecha') is-invalid @enderror" id="fecha" name="fecha" value="{{ $informe->fecha }}" required>
                    @error('fecha')
                ...
                <div class="mb-4">
                    ...
                    <select name="tipo_informe" id="tipo_informe" class="form-control @error('tipo_informe') is-invalid @enderror" required>
                        <option value="1" {{ $informe->tipo_informe == 1 ? 'selected' : '' }}>Informe de Ventas</option>
                        <option value="2" {{ $informe->tipo_informe == 2 ? 'selected' : '' }}>Informe de Compras</option>
                        <option value="3" {{ $informe->tipo_informe == 3 ? 'selected' : '' }}>Informe de Inventario</option>
                    </select>
                    @error('tipo_informe')
                        <div class="invalid-feedback">
                            {{ $message }}
                ...
                <div class="mb-4">
                    ...
                    <textarea class="form-control @error('datos_json') is-invalid @enderror" id="datos_json" name="datos_json" rows="4" required>{{ $informe->datos_json }}</textarea>
                    @error('datos_json')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
